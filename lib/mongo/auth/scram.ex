defmodule Mongo.Auth.SCRAM do
  ...
  defp first(
         %{"conversationId" => conversationId, "payload" => server_payload, "done" => false},
         first_bare,
         username,
         password,
         client_nonce
    ...
    message = [saslContinue: 1, conversationId: conversationId, payload: client_final_message]

    {message, server_signature}
  end

  defp second(%{"conversationId" => conversationId, "payload" => payload}, signature) do
    params = parse_payload(payload)
    ^signature = Base.decode64!(params["v"])
    [saslContinue: 1, conversationId: conversationId, payload: %BSON.Binary{binary: ""}]
  end
